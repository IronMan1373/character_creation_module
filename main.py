...
class Character:
    ...
    def __str__(self):   #напечатать описание класса, воспользуйтесь переопределением стандартного метода __str__
        #Чтобы вывести имя у объекта класса есть специальный атрибут __class__.__name__ , 
        #а для описания создайте ещё одну константу, назовите её BREIF_DESC_CHAR_CLASS
...
def start_training(character):
    ...
    while cmd != 'skip':
        ...
        if cmd in commands:
            char_command = commands[cmd](character)
            print(char_command)
...
def choice_char_class(char_name: str) -> Character:
    ...
    return char_class
    # Теперь функция возвращает не строку, а объект класса с переданным ему именем персонажа, 
    # поэтому аннотация объявления функции другая.
    # Ввод пользователя записывается в отдельную строковую переменную selected_class.
    # Объект выбранного пользователем класса записывается в переменную char_class.
